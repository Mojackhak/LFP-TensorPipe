...
def split_tensor4d_to_nested_df(
    ...
    if tensor.ndim != 4:
        raise ValueError(f"tensor must be 4D (epoch, channel, freq, time), got ndim={tensor.ndim}")

    n_e, n_c, n_f, n_t = tensor.shape

    if len(epoch) != n_e:
        raise ValueError(f"len(epoch)={len(epoch)} does not match tensor.shape[0]={n_e}")
    if len(channel) != n_c:
        raise ValueError(f"len(channel)={len(channel)} does not match tensor.shape[1]={n_c}")
    if len(freq) != n_f:
