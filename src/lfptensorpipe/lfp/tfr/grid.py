    ...
    *,
    ...
    if method_l == "morlet":
        if time_resolution_s is not None:
            n_cycles_vec = float(time_resolution_s) * np.pi * freqs / np.sqrt(2.0 * np.log(2.0))
            cycles_source = "morlet_fwhm_time_const"
        else:
            if cycles_strategy == "fwhm_power":
                if not (isinstance(fwhm_power_hz, (int, float)) and float(fwhm_power_hz) > 0):
                    raise ValueError("For cycles_strategy='fwhm_power', provide positive `fwhm_power_hz`.")
    ...
    **legacy_freq_grid_kwargs: Any,
    ...
    if isinstance(data, mne.Epochs):
        if method_l == "morlet":
            ...
            fwhm_time_est = np.array([mne_fwhm(f, n) for f, n in zip(freqs_use, n_cycles_vec)])
    ...
    if isinstance(power, np.ndarray):
        ...
        times_axis = np.asarray(data.times[::decim_eff]) if decim_eff is not None else np.asarray(data.times)
    else:
    ...
    if power_data.ndim == 4:
        ...
        n_epochs_meta, n_channels_meta, n_freqs_meta, n_times_meta = 1, *power_data.shape
    else:  # pragma: no cover
        n_epochs_meta = n_channels_meta = n_freqs_meta = n_times_meta = None

    metadata = dict(
        axes=dict(
            epoch=(np.arange(int(n_epochs_meta), dtype=int) if n_epochs_meta is not None else None),
            channel=np.array(ch_names, dtype=object),
            freq=np.asarray(freqs_use, dtype=float),
            time=np.asarray(times_axis, dtype=float),
            shape=(int(n_epochs_meta), int(n_channels_meta), int(n_freqs_meta), int(n_times_meta))
        ...
        params=dict(
            ...
            min_cycles=min_cycles,
            max_cycles=max_cycles,
            time_bandwidth=(float(time_bandwidth) if method_l == "multitaper" else None),
            hop_s=hop_s,
            hop_s_eff=hop_s_eff,
            decim=decim,
            decim_eff=decim_eff,
